...
/**
 * Subscription templates (doc 09 §9.5), rendered inside the Cards screen.
 * They materialize monthly card charges that settle via the card's fatura.
 */
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          if (!deleting) return;
          // Optimistic: the row leaves the table at once; close before settle.
          ...
          del.mutate(
            ...
            {
              onSuccess: () =>
                toast.success(t("subscriptions.deletedToast", { name: template.name })),
              onError: (error) => toast.error(error.message),
            },
          );
        }}
