...
function markPaid(
  row: BillRow,
  opts: { fromAccountId?: string; withoutAccount?: boolean; amount?: number } = {},
): BillRow {
  const withoutAccount =
    opts.withoutAccount ?? !(opts.fromAccountId ?? row.sourceAccountId);
  return {
    ...row,
    // A pay-time discount replaces the bill's value (doc 04 §4.16b).
    amount: opts.amount ?? row.amount,
    paid: true,
    paidAt: new Date().toISOString(),
    paidWithoutAccount: withoutAccount,
    paidFromAccountId: withoutAccount ? null : (opts.fromAccountId ?? row.sourceAccountId),
  };
}
...
export const billMutations = {
  ...
  pay: () =>
    trpc.bills.pay.mutationOptions({
      onMutate: (vars) =>
        applyOptimistic([trpc.bills.list.pathKey(), trpc.dashboard.pathKey()], () => [
          ...patchBillLists((row) => (row.id === vars.id ? markPaid(row, vars) : row)),
          ...dropFromDashboard(new Set([vars.id])),
        ]),
      onError: rollback,
      onSettled: invalidateOnSettle(moneyKeys()),
        ...
        return applyOptimistic([trpc.bills.list.pathKey(), trpc.dashboard.pathKey()], () => [
          ...
          ...patchBillLists((row) =>
            ids.has(row.id) && !row.paid && !row.wontPay && !row.creditCardId
              ? markPaid(row, { fromAccountId: vars.fromAccountId })
              : row,
          ),
    ...
    trpc.cards.create.mutationOptions({
      onMutate: (vars) =>
        applyOptimistic([trpc.cards.pathKey()], () => {
          const now = new Date().toISOString();
          // A card may have no host account (doc 04 §4.16a).
          const account = accountRef(vars.bankAccountId);
          const row: CardRow = {
            id: tempId(),
            userId: "",
